...
export default function TeamSection() {
  ...
  const loading = loadingStates.teamStats;
  const url = "https://shopmark.fr";
  ...
  useEffect(() => {
    ...
    // Précharger les filleuls en arrière-plan
    const preloadFilleuls = async () => {
      try {
        const store = useTeamStore.getState();
        await store.fetchFilleulsData(user.uid, false);
      } catch (error) {
        // Ignorer les erreurs de préchargement
      }
    };
    
    const timer = setTimeout(preloadFilleuls, 1000);
  ...
  const handleRefresh = async () => {
    if (user?.uid) {
      invalidateCache('teamStats');
      try {
        await fetchTeamData(user.uid, true);
      } catch (error) {
  ...
  if (loading && !teamData.invitationCode) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white p-4 md:p-6 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 transition-colors"
            >
              <svg 
            ...
            <Link 
              href="/mes-filleuls"
              className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl hover:from-orange-600 hover:to-amber-600 transition-all shadow-md hover:shadow-lg active:scale-95"
            >
              <List className="w-5 h-5" />
              <span className="font-semibold">Voir mes filleuls</span>
              <ChevronRight className="w-5 h-5" />
