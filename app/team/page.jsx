...
//   // Fonction pour calculer récursivement les membres et investissements
//   const calculateTeamDataRecursive = async (userId, currentLevel = 1, maxLevel = 3) => {
//     if (currentLevel > maxLevel) return { count: 0, investment: 0, userIds: [] };
...
//     // Pour chaque utilisateur, récupérer le premier investissement
//     for (const levelUserId of userIds) {
//       const userLevelsQuery = query(
//         collection(db, 'user_levels'),
//         where('userId', '==', levelUserId)
//       );
//       const userLevelsSnap = await getDocs(userLevelsQuery);
      
//       if (!userLevelsSnap.empty) {
//         // Trouver le premier investissement
//         let premierInvest = null;
        
//         // Chercher le flag isFirstInvestment
//         premierInvest = userLevelsSnap.docs.find(doc => doc.data().isFirstInvestment === true);
        
//         // Sinon, prendre le plus ancien
//         if (!premierInvest && userLevelsSnap.docs.length > 0) {
//           const sorted = userLevelsSnap.docs.sort((a, b) => {
//             const dateA = a.data().startDate?.toDate?.() || new Date(0);
//             const dateB = b.data().startDate?.toDate?.() || new Date(0);
//             return dateA - dateB;
//           });
//           premierInvest = sorted[0];
//         }
        
//         if (premierInvest) {
//           totalInvestment += premierInvest.data().investedAmount || 0;
//         }
//       }
//     }
    
//     // Récupérer les niveaux suivants (récursif)
//     let nextLevelData = { count: 0, investment: 0, userIds: [] };
//     if (currentLevel < maxLevel && userIds.length > 0) {
//       // Pour chaque utilisateur de ce niveau, calculer les niveaux suivants
//       for (const levelUserId of userIds) {
//         const childData = await calculateTeamDataRecursive(levelUserId, currentLevel + 1, maxLevel);
//         nextLevelData.count += childData.count;
//         nextLevelData.investment += childData.investment;
//         nextLevelData.userIds.push(...childData.userIds);
//       }
//     }
    
//     return {
//       count: snapshot.docs.length + nextLevelData.count,
//       investment: totalInvestment + nextLevelData.investment,
//       userIds: [...userIds, ...nextLevelData.userIds],
//       // Données spécifiques à ce niveau
//       levelCount: snapshot.docs.length,
//       levelInvestment: totalInvestment
//     };
//   };

//   // Calculer les données spécifiques par niveau
//   const calculateLevelData = async (userId) => {
//     const result = {
...
//     );
//     const level1Snap = await getDocs(level1Query);
//     const level1Ids = level1Snap.docs.map(doc => doc.id);
//     result.level1.count = level1Ids.length;
    
//     // Calcul investissement niveau 1
//     for (const level1Id of level1Ids) {
//       const userLevelsQuery = query(
//         collection(db, 'user_levels'),
//         where('userId', '==', level1Id)
//       );
//       const userLevelsSnap = await getDocs(userLevelsQuery);
      
//       if (!userLevelsSnap.empty) {
//         let premierInvest = userLevelsSnap.docs.find(doc => doc.data().isFirstInvestment === true);
//         if (!premierInvest && userLevelsSnap.docs.length > 0) {
//           const sorted = userLevelsSnap.docs.sort((a, b) => {
//             const dateA = a.data().startDate?.toDate?.() || new Date(0);
//             const dateB = b.data().startDate?.toDate?.() || new Date(0);
...
//         if (premierInvest) {
//           result.level1.investment += premierInvest.data().investedAmount || 0;
//         }
//       }
//     }
...
//       const level2Query = query(
//         collection(db, 'users'),
//         where('referrerId', 'in', level1Ids.slice(0, 10)) // Limite à 10 pour Firestore
//       );
//       const level2Snap = await getDocs(level2Query);
//       const level2Ids = level2Snap.docs.map(doc => doc.id);
//       result.level2.count = level2Ids.length;
      
//       // Calcul investissement niveau 2
...
//           if (premierInvest) {
//             result.level2.investment += premierInvest.data().investedAmount || 0;
//           }
//         }
//       }
...
//         const level3Query = query(
//           collection(db, 'users'),
//           where('referrerId', 'in', level2Ids.slice(0, 10))
//         );
//         const level3Snap = await getDocs(level3Query);
//         const level3Ids = level3Snap.docs.map(doc => doc.id);
//         result.level3.count = level3Ids.length;
        
//         // Calcul investissement niveau 3
//         for (const level3Id of level3Ids) {
//           const userLevelsQuery = query(
//             collection(db, 'user_levels'),
...
//             if (premierInvest) {
//               result.level3.investment += premierInvest.data().investedAmount || 0;
//             }
//           }
//         }
//       }
...
//         // 2. Calculer les données par niveau
//         const levelData = await calculateLevelData(user.uid);
        
//         // 3. Calculer les commissions totales
...
//           const data = doc.data();
//           totalCommissionEarned += data.amount || 0;
//         });

//         // 4. Préparer les niveaux de commission
//         const commissionRates = [3, 2, 1]; // 3%, 2%, 1%
//         const levelNames = ["Menbre A", "Menbre B", "Menbre C"];
//         const colors = [
//           { gradient: "from-orange-500 to-amber-400", iconColor: "text-orange-500" },
...
//           const validUsers = levelInfo.count;
//           const totalInvestment = levelInfo.investment;
//           const revenue = Math.round(totalInvestment * (commissionRates[index] / 100));
...
//         // 5. Calculer le total des membres
//         const totalMembers = levelData.level1.count + levelData.level2.count + levelData.level3.count;

//         // 6. Récupérer le revenu total (commissions + autres revenus)
//         const walletDoc = await getDoc(doc(db, 'wallets', user.uid));
//         const walletData = walletDoc.exists() ? walletDoc.data() : {};
//         const totalRevenue = totalCommissionEarned + (walletData.stats?.totalEarned || 0);

//         setTeamData({
//           invitationCode,
//           invitationLink,
//           teamMembers: {
//             level1: levelData.level1.count,
//             level2: levelData.level2.count,
//             level3: levelData.level3.count,
//             total: totalMembers
//           },
//           totalRevenue,
...
//           levels: levels.map(l => ({ level: l.level, users: l.validUsers, revenue: l.revenue }))
//         });
//       } catch (error) {
//         console.error('❌ Erreur chargement données équipe:', error);
//       } finally {
...
//                 {/* Information supplémentaire avec calcul explicite */}
//                 {level.validUsers > 0 && level.totalInvestment > 0 && (
//                   <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
//                     <p className="text-sm text-blue-700 mb-1 font-medium">
//                       💡 Calcul détaillé :
//                     </p>
//                     <p className="text-xs text-blue-600">
//                       {level.validUsers} membre(s) × {formatCurrency(level.totalInvestment)} investis × {level.commissionRate}%
//                       {level.revenue > 0 ? ` = ${formatCurrency(level.revenue)}` : ''}
//                     </p>
//                   </div>
//                 )}
...
//                   ✅ Synchronisation : Les données affichées ici sont exactement les mêmes que dans "Mes Filleuls"
//                 </p>
//               </div>
//               <p className="text-sm text-gray-600 mt-2">
//                 Les commissions sont versées automatiquement sur votre portefeuille 
