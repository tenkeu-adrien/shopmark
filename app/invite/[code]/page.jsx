...
//       // Validation du code d'invitation (s'il est fourni)
...
import { useRouter, useSearchParams } from 'next/navigation';
import { usePathname } from 'next/navigation';
import { Lock, Gift, Check, AlertCircle, Eye, EyeOff, Mail, User, Globe, Phone } from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext';
import Link from 'next/link';
import Image from 'next/image';
...
export default function AuthPage() {
  const [isLogin, setIsLogin] = useState(true);
  const [authMethod, setAuthMethod] = useState('phone'); // 'phone' ou 'email'
  const [countryCode, setCountryCode] = useState('+243'); // Code pays saisissable manuellement
  const [email, setEmail] = useState("");
  const [fullName, setFullName] = useState('');
  const [phone, setPhone] = useState("");
  ...
  // Réinitialiser les champs quand on change de méthode d'authentification
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      ...
      if (authMethod === 'phone') {
        ...
        try {
          const result = await login(fullPhoneNumber, password, 'phone');
          if (result.success) {
            router.push('/');
          } else {
            setError('Numéro de téléphone ou mot de passe incorrect');
          ...
          setError('Erreur lors de la connexion par téléphone');
          console.error(err);
        }
      } else {
        // Connexion par email
        ...
        try {
          const result = await login(cleanedEmail, password, 'email');
          if (result.success) {
            router.push('/');
          } else {
      ...
      // Pour l'inscription, on exige toujours le téléphone
      if (!cleanedCountryCode.startsWith('+')) {
        setError('Le code pays doit commencer par + (ex: +243)');
        setIsLoading(false);
        return;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-black py-12 px-4 sm:px-6 lg:px-8">
      <div className="relative max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          {/* Sélecteur de méthode d'authentification (connexion uniquement) */}
          {isLogin && (
          ...
          {/* Email (toujours requis pour inscription, optionnel pour connexion selon méthode) */}
          <div className={`${isLogin && authMethod === 'phone' ? 'hidden' : ''}`}>
            <label htmlFor="email" className="block text-sm font-medium text-gray-300 mb-2">
              Adresse email {!isLogin && <span className="text-red-500">*</span>}
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <Mail className="h-5 w-5 text-gray-500" />
          ...
          {/* Numéro de téléphone avec code pays manuel (toujours requis pour inscription, optionnel pour connexion selon méthode) */}
          <div className={`${isLogin && authMethod === 'email' ? 'hidden' : ''}`}>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              Numéro de téléphone {!isLogin && <span className="text-red-500">*</span>}
            </label>
            <div className="flex gap-3">
              {/* Champ code pays manuel */}
              <div className="relative flex-shrink-0">
