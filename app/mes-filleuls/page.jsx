...
export default function MesFilleulsPage() {
  ...
  const loading = loadingStates.filleuls || authLoading;
  const filleuls = filleulsData.filleuls || [];
  const stats = filleulsData.stats || {
    total: 0,
    actifs: 0,
    inactifs: 0,
    totalInvesti: 0,
  ...
  const loadFilleulsData = async () => {
    ...
    try {
      await fetchFilleulsData(user.uid);
    } catch (error) {
      console.error('❌ Erreur chargement filleuls:', error);
    }
  };

  // Filtrage local (identique à avant)
  const filteredFilleuls = filleuls.filter(filleul => {
    if (searchTerm) {
      const searchLower = searchTerm.toLowerCase();
      return (
  ...
  const handleRefresh = () => {
    if (user?.uid) {
      invalidateCache('filleuls');
      fetchFilleulsData(user.uid, true);
    }
  };
  ...
  if (authLoading || (loading && filleuls.length === 0)) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
          ...
          <div className="mt-4 text-sm text-gray-500 flex flex-wrap gap-4">
            ...
            {stats.totalBonus > 0 && (
              <>
                <span className="hidden sm:inline">•</span>
                <span className="text-amber-600 font-medium text-[18px]">
                  Bonus total: {formatCurrency(stats.totalBonus)}
                </span>
              </>
            )}
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white rounded-lg p-4 border border-amber-200">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-amber-900">Bonus total reçu</p>
                      <p className="text-2xl font-bold text-amber-700">{formatCurrency(stats.totalBonus)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg p-4 border border-amber-200">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-green-700">
                        {stats.totalInvesti > 0 
                          ? `${((stats.totalBonus / stats.totalInvesti) * 100).toFixed(1)}%`
                          : '0%'
                        }
                      </p>
                      <p className="text-xs text-green-700 mt-1">Retour sur investissements</p>
